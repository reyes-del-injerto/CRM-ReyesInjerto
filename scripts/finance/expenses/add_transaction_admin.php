...
try {
    ...
    $sub_sub_cat_id = $_POST['sub_sub_cat_id'];  // Nuevo campo sub_subcategoría
    $clinic = $_POST['clinic'];
    $created_by = $_POST['user_id'];

    // Verifica si el valor de subcategoría es un número entero
    ...
    $insertSql = $conn->prepare("
        INSERT INTO ad_transactions (description, payment_method_id, amount, date, store, cat_id, current_status, subcategory, sub_subcategory, clinic, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $insertSql->bind_param("sidssiiiisi", $description, $payment_method_id, $negativeAmount, $date, $store, $cat_id, $current_statuus, $sub_cat_id, $sub_sub_cat_id, $clinic, $created_by);
