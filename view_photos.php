...
if (isset($_GET['type']) && $_GET['type'] == "procedure" && isset($_GET['id'])) {
	$procedure_id = $_GET['id'];

	$sql_row = "SELECT CONCAT(sla.first_name, ' ', sla.last_name) AS name, sla.procedure_date, sla.procedure_type, sla.enfermedades, ep.clinic, ep.num_med_record, ep.touchup, ep.room, ep.specialist, ep.notes FROM enf_procedures ep INNER JOIN sa_leads_assessment sla ON ep.lead_id = sla.lead_id WHERE /* sla.status = 1 AND */ ep.lead_id = ?  AND sla.status != 0";
	...
	if ($px_info->enfermedades === "ninguna" || is_null($px_info->enfermedades) || $px_info->enfermedades === "") {
		$enfermedadesTitle = "ninguna";
	} else {
		$enfermedadesTitle = $px_info->enfermedades;
	}
			...
			<div class="text-center col-md-12 col-xs-12 order-md-1 order-last">
				<div class="card text-white bg-secondary ">
					<div class="card-body text-center">
						...
						<p><span style="font-size:12px;" class="badge bg-dark"> Enfermedades reportadas:' . $enfermedadesTitle . '</span>
...
} else if (isset($_GET['type']) && $_GET['type'] == "touchup" && isset($_GET['id'])) {
	// !! FALTA AÑADIR LA VISTA EN RETOQUES (TOUCHUP)
}
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="mini-sidebar">
					...
					<div class="row">
						<div class="col-sm-12">
							<ul class="breadcrumb">
								...
								<li class="breadcrumb-item"><i class="fa fa-arrow-right"></i></li>
								<li class="breadcrumb-item active"><a class="nav-link active" href="#">Fotografías de Revisiones</a></li>
						</div>
					</div>
				</div>
	...
	<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
				...
				<div class="modal-body">
					<form id="addNoteForm" method="post">
						<div class="form-group">
							...
							<!-- <input type="text" class="form-control" id="clinic_note" name="clinic" required> -->
						</div>
		...
		`;
				...
				function showSign(step, num_med_record, clinic) {
					console.log("clinica recibida en showSign", clinic)
					$.ajax({
	...
	`;
								...
								$botonCrearPdf.addEventListener("click", async () => {
									...
									// Obtener el valor del campo clinic
									let inputValueClinic = $("#clinic").val();
									console.log("clinic value", inputValueClinic);
									...
									formData.append("clinic", inputValueClinic);
									...
									try {
										...
										showSign(step, num_med_record, clinic);
										showSweetAlert(
											"Listo!",
											"Firma agregada",
											"success",
